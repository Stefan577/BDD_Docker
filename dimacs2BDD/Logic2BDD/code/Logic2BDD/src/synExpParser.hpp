...
#ifndef YY_YY_SRC_SYNEXPPARSER_HPP_INCLUDED
# define YY_YY_SRC_SYNEXPPARSER_HPP_INCLUDED
// "%code requires" blocks.
#line 55 "../tools/src/synExpParser.ypp"
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    T_SYMBOL = 261,                // T_SYMBOL
    T_TRUE = 262,                  // T_TRUE
    T_FALSE = 263,                 // T_FALSE
    T_MODULE = 264,                // T_MODULE
    T_IF = 265,                    // T_IF
    T_THEN = 266,                  // T_THEN
    T_ELSE = 267,                  // T_ELSE
    T_UNEQUAL = 268,               // T_UNEQUAL
    T_CLOSEPAREN = 269,            // T_CLOSEPAREN
    T_OPENPAREN = 270,             // T_OPENPAREN
    T_EOL = 271,                   // T_EOL
    T_XOR = 272,                   // T_XOR
    T_IMPLIES = 273,               // T_IMPLIES
    T_OR = 274,                    // T_OR
    T_AND = 275,                   // T_AND
    T_EQUAL = 276,                 // T_EQUAL
    T_NOT = 277                    // T_NOT
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_T_MENU = 4,                            // T_MENU
        S_T_LITERAL = 5,                         // T_LITERAL
        S_T_SYMBOL = 6,                          // T_SYMBOL
        S_T_TRUE = 7,                            // T_TRUE
        S_T_FALSE = 8,                           // T_FALSE
        S_T_MODULE = 9,                          // T_MODULE
        S_T_IF = 10,                             // T_IF
        S_T_THEN = 11,                           // T_THEN
        S_T_ELSE = 12,                           // T_ELSE
        S_T_UNEQUAL = 13,                        // T_UNEQUAL
        S_T_CLOSEPAREN = 14,                     // T_CLOSEPAREN
        S_T_OPENPAREN = 15,                      // T_OPENPAREN
        S_T_EOL = 16,                            // T_EOL
        S_T_XOR = 17,                            // T_XOR
        S_T_IMPLIES = 18,                        // T_IMPLIES
        S_T_OR = 19,                             // T_OR
        S_T_AND = 20,                            // T_AND
        S_T_EQUAL = 21,                          // T_EQUAL
        S_T_NOT = 22,                            // T_NOT
        S_YYACCEPT = 23,                         // $accept
        S_explist = 24,                          // explist
        S_exp = 25,                              // exp
        S_seqexp = 26                            // seqexp
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_seqexp: // seqexp
        value.move< std::vector<synExp*> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::vector<synExp*>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
...
switch (yykind)
    {
      ...
      {
        return synExpParser::symbol_name (this->kind ());
      }
...
#if 201103L <= YY_CPLUSPLUS
    /// Non copyable.
    synExpParser (const synExpParser&) = delete;
    /// Non copyable.
    synExpParser& operator= (const synExpParser&) = delete;
#endif
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_T_LITERAL (std::string v, location_type l)
      {
        return symbol_type (token::T_LITERAL, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_T_LITERAL (const std::string& v, const location_type& l)
      {
        return symbol_type (token::T_LITERAL, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_T_SYMBOL (std::string v, location_type l)
      {
        return symbol_type (token::T_SYMBOL, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_T_SYMBOL (const std::string& v, const location_type& l)
      {
        return symbol_type (token::T_SYMBOL, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_T_CLOSEPAREN (location_type l)
      {
        return symbol_type (token::T_CLOSEPAREN, std::move (l));
      }
#else
      static
      symbol_type
      make_T_CLOSEPAREN (const location_type& l)
      {
        return symbol_type (token::T_CLOSEPAREN, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_T_OPENPAREN (location_type l)
      {
        return symbol_type (token::T_OPENPAREN, std::move (l));
      }
#else
      static
      symbol_type
      make_T_OPENPAREN (const location_type& l)
      {
        return symbol_type (token::T_OPENPAREN, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_T_XOR (location_type l)
      {
        return symbol_type (token::T_XOR, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_T_IMPLIES (location_type l)
      {
        return symbol_type (token::T_IMPLIES, std::move (l));
      }
#else
      static
      symbol_type
    ...
    {
    public:
      context (const synExpParser& yyparser, const symbol_type& yyla);
...
#if YY_CPLUSPLUS < 201103L
    /// Non copyable.
    synExpParser (const synExpParser&);
    /// Non copyable.
    synExpParser& operator= (const synExpParser&);
#endif
  ...
  private:
    ...
    // User arguments.
    class synExpDriver& driver;

  };

  inline
  synExpParser::symbol_kind_type
  synExpParser::yytranslate_ (int t) YY_NOEXCEPT
  {
  ...
  // basic_symbol.
  template <typename Base>
  synExpParser::basic_symbol<Base>::basic_symbol (const basic_symbol& that)
    : Base (that)
    , value ()
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_seqexp: // seqexp
        value.copy< std::vector<synExp*> > (YY_MOVE (that.value));
        break;
  ...
  template <typename Base>
  synExpParser::symbol_kind_type
  synExpParser::basic_symbol<Base>::type_get () const YY_NOEXCEPT
  {
    return this->kind ();
  }
  ...
  template <typename Base>
  bool
  synExpParser::basic_symbol<Base>::empty () const YY_NOEXCEPT
  {
  ...
  template <typename Base>
  void
  synExpParser::basic_symbol<Base>::move (basic_symbol& s)
  {
    super_type::move (s);
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_seqexp: // seqexp
        value.move< std::vector<synExp*> > (YY_MOVE (s.value));
        break;
  ...
  // by_kind.
  inline
  synExpParser::by_kind::by_kind () YY_NOEXCEPT
    : kind_ (symbol_kind::S_YYEMPTY)
  {}

#if 201103L <= YY_CPLUSPLUS
  inline
  synExpParser::by_kind::by_kind (by_kind&& that) YY_NOEXCEPT
    : kind_ (that.kind_)
  {
    that.clear ();
  }
  ...
  inline
  synExpParser::by_kind::by_kind (const by_kind& that) YY_NOEXCEPT
    : kind_ (that.kind_)
  {}

  inline
  synExpParser::by_kind::by_kind (token_kind_type t) YY_NOEXCEPT
  ...
  inline
  void
  synExpParser::by_kind::clear () YY_NOEXCEPT
  {
    kind_ = symbol_kind::S_YYEMPTY;
  }
  ...
  inline
  void
  synExpParser::by_kind::move (by_kind& that)
  {
    kind_ = that.kind_;
    that.clear ();
  }
  ...
  inline
  synExpParser::symbol_kind_type
  synExpParser::by_kind::kind () const YY_NOEXCEPT
  {
    return kind_;
  }
  ...
  inline
  synExpParser::symbol_kind_type
  synExpParser::by_kind::type_get () const YY_NOEXCEPT
  {
    return this->kind ();
  }
...
#line 1730 "src/synExpParser.hpp"




#endif // !YY_YY_SRC_SYNEXPPARSER_HPP_INCLUDED
